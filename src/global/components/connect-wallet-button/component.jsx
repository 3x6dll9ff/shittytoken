...
import "./css/connect-wallet-button.css";
...
const ConnectWalletButton = ({wallet, connectType, onWalletConnect}) => {
    ...
    return (
        <div
            className={`connect-wallet-button ${walletInstalled(wallet) ? '' : 'disabled'}`}
            onClick={walletInstalled(wallet) ? () => connectWallet(wallet, connectType, onWalletConnect) : null}
        >
            <div className={`connect-wallet-button-icon-container`}>
                {okxReplaces(wallet) ? (
                    <img
                        className={`connect-wallet-button-icon right ${walletInstalled(wallet) ? '' : 'disabled'}`}
                        src={okx_icon}
                        alt={`header-profile-wallet-popup-button-icon`}
                    />
                ) : null}
                <img
                    className={`connect-wallet-button-icon ${okxReplaces(wallet) ? 'left' : ''} ${walletInstalled(wallet) ? '' : 'disabled'}`}
                    src={walletIcons[wallet]}
                    alt={`header-profile-wallet-popup-button-icon`}
                />
            </div>
            <div
                className={`connect-wallet-button-wallet-name-container ${walletInstalled(wallet) ? '' : 'disabled'}`}>
                <div
                    className={`connect-wallet-button-wallet-name ${walletInstalled(wallet) ? '' : 'disabled'}`}>
                    {walletNames[wallet] || wallet} {okxReplaces(wallet) ? '/ OKX Wallet' : ''}
                </div>
                <div
                    className={`connect-wallet-button-wallet-status ${walletInstalled(wallet) ? 'green' : 'red'}`}>
